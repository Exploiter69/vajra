...
class RunManager:
    ...
    def create_run(self, run: EngineeringRun) -> EngineeringRun:
        with self._lock:
            ...
            try:
                self._record_event(run_id=run.run_id, event_type="RUN_CREATED")
            except Exception:
    ...
    def transition(
        ...
        *,
        ...
        with self._lock:
            ...
            self._transition_authority.assert_authorized(run, target, actor, reason=reason)
            ...
            try:
                ...
                self._record_event(
                    ...
                    payload={"from": previous_state.value, "to": target.value},
                )
                return result
            except Exception:
    ...
    def abort_run(self, run_id: str, reason: str, actor: TransitionActor) -> EngineeringRun:
        with self._lock:
            run = self.get_run(run_id)
            previous = deepcopy(run)
            ...
            try:
                result = self._state_store.save_run(run)
                self._record_event(run_id=run_id, event_type="RUN_ABORTED", payload={"reason": reason})
                return result
            except Exception:
                self._state_store.save_run(previous)
                raise
    ...
    def add_step(self, run_id: str, step_id: str, name: str) -> Step:
        with self._lock:
            ...
            step = Step(step_id=step_id, run_id=run_id, name=name)
    ...
    def start_attempt(
        ...
        *,
        ...
        with self._lock:
            ...
            if any(attempt.attempt_id == attempt_id for attempt in step.attempts):
                raise ValueError(f"Attempt already exists: {attempt_id}")
            lease = self._lease_manager.acquire(attempt_id, worker_id, lease_id, lease_ttl_seconds, now=now)
            attempt = Attempt(
            ...
            try:
                ...
                self._state_store.save_run(previous)
                self._lease_manager.release(attempt_id, worker_id, lease.lease_id, lease.fencing_token, now=now)
                raise
    ...
    def fail_attempt(self, run_id: str, step_id: str, attempt_id: str, error: str) -> Attempt:
        with self._lock:
            run = self.get_run(run_id)
            previous = deepcopy(run)
            step = self._get_step(run, step_id)
            ...
            if attempt.state is not AttemptState.RUNNING:
                raise ValueError(f"Attempt is not running: {attempt.attempt_id}")
            ...
            if run.state is not RunState.RECOVERING:
                ...
                try:
                    ...
                    self._record_event(
                        ...
                        payload={"from": previous_state.value, "to": RunState.RECOVERING.value},
                    )
                    return run
                except Exception:
                    self._state_store.save_run(previous)
