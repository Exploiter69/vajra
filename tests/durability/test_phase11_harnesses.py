...
def test_soak_runner_uses_real_duration_contract_without_sleeping_in_unit_test(tmp_path: Path):
    clock = [0.0]
    counter = [0]
    def monotonic() -> float:
        return clock[0]
    ...
    metrics = SoakRunner(SoakConfig(10.0, 2.0, tmp_path), counter_provider=counters, workload=workload, monotonic=monotonic, sleep=sleep).run()
...
def test_soak_runner_persists_complete_evidence_record(tmp_path: Path):
    ...
    metrics = SoakRunner(SoakConfig(4.0, 2.0, tmp_path), counter_provider=lambda: SoakCounters(event_count=2), monotonic=monotonic, sleep=sleep).run_and_write(evidence)
...
def test_soak_runner_rejects_negative_runtime_counters(tmp_path: Path):
    ...
    runner = SoakRunner(SoakConfig(1.0, 1.0, tmp_path), counter_provider=counters, monotonic=lambda: 0.0, sleep=lambda _seconds: None)
    try:
        runner.run()
...
def _make_reality_run(revision: str, objective: str = "detect reality divergence") -> EngineeringRun:
    return EngineeringRun(run_id="phase11-reality", objective=objective, repository_id="repo", base_revision=revision, acceptance_criteria=("tracked.txt remains present",), policy_id="policy", policy_version="1", budget_id="budget", created_by="test")
...
def test_reality_observer_detects_actual_git_and_filesystem_divergence(tmp_path: Path):
    ...
    contract = SimpleNamespace(workspace_id="workspace-1", base_revision=revision, path=str(workspace), git_status_digest=status_digest)
    clean = RealityObserver().observe(run=run, worktree=contract, git_revision=revision, git_status=status, active_lease_state="VALID", verification_state="PASS", budget_state="AVAILABLE")
    ...
    divergent = RealityObserver().observe(run=run, worktree=contract, git_revision=revision, git_status=divergent_status, active_lease_state="VALID", verification_state="PASS", budget_state="AVAILABLE")
...
def test_reality_observer_catches_three_source_divergence(tmp_path: Path):
    ...
    contract = SimpleNamespace(workspace_id="workspace-three-source", base_revision=base_revision, path=str(workspace), git_status_digest=hashlib.sha256(clean_status.encode("utf-8")).hexdigest())
    ...
    report = RealityObserver().observe(run=current_run, worktree=contract, git_revision=git_revision, git_status=git_status, active_lease_state="VALID", verification_state="PASS", budget_state="AVAILABLE", expected_durable_state_digest=stable_digest(durable_checkpoint))
...
def test_autonomous_loop_retry_storm_is_bounded_at_control_plane(tmp_path: Path):
    ...
    class AlwaysFailBackend:
        def execute(self, request):
            return ExecutionResult(status=ExecutionStatus.REJECTED, operation=request.intent.operation, errors=("persistent deterministic failure",))
    ...
    assert any(event.event_type == "AUTONOMY_PLAN" for event in manager.events("phase10-run"))
    assert any(event.event_type == "AUTONOMY_OBSERVE" for event in manager.events("phase10-run"))
